...
## Onboarding advisor #N

1. Add `packages/clients/src/configs/<advisor>.ts` exporting a `ClientConfig`.
2. Register it in `CLIENT_CONFIGS` (in `registry.ts`).
3. That's it — a demo tenant seeds automatically, and the config appears at
   `/client`. To boot the app into that advisor, set
   `AION_ACTIVE_CLIENT=<clientId>`.
4. To take them live, complete their approval/compliance sign-off and satisfy
   the runtime-mode activation gates (see [`docs/environments.md`](environments.md)).
